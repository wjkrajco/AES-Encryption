/** 
    @file aes.c
    @author William J Krjacovic (wjkrajco)
    This is aes.c which is the file that is responsible
    for the decryption and encryption based on given keys using the AES method.
*/

#include <stdbool.h>
#include <stdlib.h>
#include <stdio.h>
#include "field.h"
#include "aes.h"

/**
   Return the sBox substitution value for a given byte value.
   @param v byte input value.
   @return substitution for the given byte.
*/
static byte substBox( byte v )
{
  // Forward-direction replacement map for the sBox.
  static const byte rule[] =
    {  };

  // Return the replaceent for v based on the map.
  return rule[ v ];
}

/**
   Return the inverse sBox substitution value for a given byte value.
   @param v byte input value.
   @return inverse substitution for the given byte.
*/
static byte invSubstBox( byte v )
{
  // Inverse-direction replacement map for the sBox.
  static const byte irule[] =
  {  };
  
  // Return the (inverse) replaceent for v based on the map.
  return irule[ v ];
}

void gFunction( byte dest[ WORD_SIZE ], byte const src[ WORD_SIZE ], int r )
{
  // Constant values used in each round of the g function.
  static const byte roundConstant[ ROUNDS + 1 ] = {
    0x00, // First element not used.
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
  };

  dest[0] =  fieldAdd(substBox(src[1]), roundConstant[r]);
  dest[1] = substBox(src[MOVE_OVER_TWO]);
  dest[MOVE_OVER_TWO] = substBox(src[MOVE_OVER_THREE]);
  dest[MOVE_OVER_THREE] = substBox(src[0]);
}  

void generateSubkeys(byte subkey[ROUNDS + 1][BLOCK_SIZE], byte const key[BLOCK_SIZE])
{
  byte result[WORD_SIZE];

  for (int i = 0; i < BLOCK_SIZE; i++) {
    subkey[0][i] = key[i];
  }

  for (int r = 1; r <= ROUNDS; r++) {
    gFunction(result, &subkey[r - 1][BLOCK_SIZE - WORD_SIZE], r);

    for (int i = 0; i < WORD_SIZE; i++) {
      subkey[r][i] = subkey[r - 1][i] ^ result[i];
    }

    for (int i = WORD_SIZE; i < BLOCK_SIZE; i++) {
      subkey[r][i] = subkey[r - 1][i] ^ subkey[r][i - WORD_SIZE];
    }
  }
}

void addSubkey(byte data[BLOCK_SIZE], byte const key[BLOCK_SIZE])
{
  byte result;
  for (int i = 0; i < BLOCK_SIZE; i++)  {
    result = fieldAdd(data[i], key[i]);
    data[i] = result;
  }
}

void blockToSquare( byte square[ BLOCK_ROWS ][ BLOCK_COLS ], byte const data[ BLOCK_SIZE ] )
{
  int counter = 0;
  for (int i = 0; i < BLOCK_COLS; i++)  {
    for (int j = 0; j < BLOCK_ROWS; j++)  {
      square[j][i] = data[counter];
      counter++;
    }
  }
}

void squareToBlock( byte data[ BLOCK_SIZE ], byte const square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
    int counter = 0;
  for (int i = 0; i < BLOCK_COLS; i++)  {
    for (int j = 0; j < BLOCK_ROWS; j++)  {
      data[counter] = square[j][i];
      counter++;
    }
  }

}

void shiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{

  byte temp = 0;

  temp = square[1][0];
  square[1][0] = square[1][1];
  square[1][1] = square[1][MOVE_OVER_TWO];
  square[1][MOVE_OVER_TWO] = square[1][MOVE_OVER_THREE];
  square[1][MOVE_OVER_THREE] = temp;

  temp = square[MOVE_OVER_TWO][0];
  square[MOVE_OVER_TWO][0] = square[MOVE_OVER_TWO][MOVE_OVER_TWO];
  square[MOVE_OVER_TWO][MOVE_OVER_TWO] = temp;
  temp = square[MOVE_OVER_TWO][1];
  square[MOVE_OVER_TWO][1] = square[MOVE_OVER_TWO][MOVE_OVER_THREE];
  square[MOVE_OVER_TWO][MOVE_OVER_THREE] = temp;

  temp = square[MOVE_OVER_THREE][0];
  square[MOVE_OVER_THREE][0] = square[MOVE_OVER_THREE][MOVE_OVER_THREE];
  square[MOVE_OVER_THREE][MOVE_OVER_THREE] = square[MOVE_OVER_THREE][MOVE_OVER_TWO];
  square[MOVE_OVER_THREE][MOVE_OVER_TWO] = square[MOVE_OVER_THREE][1];
  square[MOVE_OVER_THREE][1] = temp;

}

void unShiftRows( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  byte temp = 0;

  temp = square[1][MOVE_OVER_THREE];
  square[1][MOVE_OVER_THREE] = square[1][MOVE_OVER_TWO];
  square[1][MOVE_OVER_TWO] = square[1][1];
  square[1][1] = square[1][0];
  square[1][0] = temp;

  temp = square[MOVE_OVER_TWO][MOVE_OVER_TWO];
  square[MOVE_OVER_TWO][MOVE_OVER_TWO] = square[MOVE_OVER_TWO][0];
  square[MOVE_OVER_TWO][0] = temp;
  temp = square[MOVE_OVER_TWO][MOVE_OVER_THREE];
  square[MOVE_OVER_TWO][MOVE_OVER_THREE] = square[MOVE_OVER_TWO][1];
  square[MOVE_OVER_TWO][1] = temp;

  temp = square[MOVE_OVER_THREE][1];
  square[MOVE_OVER_THREE][1] = square[MOVE_OVER_THREE][MOVE_OVER_TWO];
  square[MOVE_OVER_THREE][MOVE_OVER_TWO] = square[MOVE_OVER_THREE][MOVE_OVER_THREE];
  square[MOVE_OVER_THREE][MOVE_OVER_THREE] = square[MOVE_OVER_THREE][0];
  square[MOVE_OVER_THREE][0] = temp;
}



void mixColumns(byte square[BLOCK_ROWS][BLOCK_COLS])
{
  // Matrix by which each column of the square is multiplied.
  static const byte mixMatrix[BLOCK_ROWS][BLOCK_COLS] = {
    {0x02, 0x03, 0x01, 0x01},
    {0x01, 0x02, 0x03, 0x01},
    {0x01, 0x01, 0x02, 0x03},
    {0x03, 0x01, 0x01, 0x02}};

  byte newColumn[BLOCK_ROWS][BLOCK_COLS];

  for (int col = 0; col < BLOCK_COLS; col++) {
    for (int row = 0; row < BLOCK_ROWS; row++) {
      newColumn[row][col] = 0;
      for (int k = 0; k < BLOCK_COLS; k++) {
        newColumn[row][col] ^= fieldMul(mixMatrix[row][k], square[k][col]);
      }
    }
  }

  for (int row = 0; row < BLOCK_ROWS; row++) {
    for (int col = 0; col < BLOCK_COLS; col++) {
      square[row][col] = newColumn[row][col];
      }
  }
}

void unMixColumns( byte square[ BLOCK_ROWS ][ BLOCK_COLS ] )
{
  // Matrix by which each column of square is multiplied.
  static const byte invMixMatrix[ BLOCK_ROWS ][ BLOCK_COLS ] = {
    { 0x0E, 0x0B, 0x0D, 0x09 },
    { 0x09, 0x0E, 0x0B, 0x0D },
    { 0x0D, 0x09, 0x0E, 0x0B },
    { 0x0B, 0x0D, 0x09, 0x0E }
  };

  byte temp[BLOCK_ROWS][BLOCK_COLS];

  for (int col = 0; col < BLOCK_COLS; col++) {
    for (int row = 0; row < BLOCK_ROWS; row++) {
      temp[row][col] = 0;
      for (int k = 0; k < BLOCK_COLS; k++) {
        temp[row][col] ^= fieldMul(invMixMatrix[row][k], square[k][col]);
      }
    }
  }

  for (int row = 0; row < BLOCK_ROWS; row++) {
    for (int col = 0; col < BLOCK_COLS; col++) {
      square[row][col] = temp[row][col];
    }
  }
}

void encryptBlock( byte data[ BLOCK_SIZE ], byte key[ BLOCK_SIZE ] )
{
  byte square[BLOCK_ROWS][BLOCK_COLS];
  byte subkey[ROUNDS + 1][BLOCK_SIZE];

  generateSubkeys(subkey, key);

  addSubkey(data, subkey[0]);

  for (int i = 1; i < ROUNDS + 1; i++) {
    for (int j = 0; j < BLOCK_SIZE; j++) {
      data[j] = substBox(data[j]);
    }

    blockToSquare(square, data);
    shiftRows(square);

    if (i < ROUNDS) {
      mixColumns(square);
    }

    squareToBlock(data, square);
    addSubkey(data, subkey[i]);
  }
}

void decryptBlock(byte data[BLOCK_SIZE], byte key[BLOCK_SIZE]) {
  byte square[BLOCK_ROWS][BLOCK_COLS];
  byte subkey[ROUNDS + 1][BLOCK_SIZE];

  generateSubkeys(subkey, key);

  for (int i = ROUNDS; i > 0; i--) {
    addSubkey(data, subkey[i]);

    blockToSquare(square, data);
    if (i != ROUNDS) {
      unMixColumns(square);
    }
    unShiftRows(square);
    squareToBlock(data, square);

    for (int j = 0; j < BLOCK_SIZE; j++) {
      data[j] = invSubstBox(data[j]);
    }
  }
  addSubkey(data, subkey[0]);
}

